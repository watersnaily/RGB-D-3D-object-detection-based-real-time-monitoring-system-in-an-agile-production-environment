...
# from provider_fpointnet import compute_box3d_iou
...
class PointNetInstanceSeg(nn.Module):
    def __init__(self,n_classes=6,n_channel=4):
...
class FrustumPointNetv1(nn.Module):
    def __init__(self,n_classes=6,n_channel=6):
        super(FrustumPointNetv1, self).__init__()
        self.n_classes = n_classes
        self.InsSeg = PointNetInstanceSeg(n_classes=6,n_channel=n_channel)
        self.STN = STNxyz(n_classes=6)
        self.est = PointNetEstimation(n_classes=6)
    ...
    def forward(self, pts, one_hot_vec):#bs,4,n
        ...
        # if torch.lt(torch.sum(mask),100):
            ...
            # logits = self.InsSeg(pts,one_hot_vec)#bs,n,2
            # object_pts_xyz, mask_xyz_mean, mask = \
            #      point_cloud_masking(pts, logits)
            # print(torch.sum(mask))
