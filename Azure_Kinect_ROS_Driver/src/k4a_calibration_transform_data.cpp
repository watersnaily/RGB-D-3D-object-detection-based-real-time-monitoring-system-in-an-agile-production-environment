...
void K4ACalibrationTransformData::getDepthCameraInfo(sensor_msgs::CameraInfo& camera_info)
{
  ...
  std::ifstream infile("../calibration/depth_camera_calibration.txt",std::ios::in);
  if (!infile.fail()){
    double num[15];
    int datalen=0;
    while( ! infile.eof() ){
    ...
    infile.close();
    parameters->param.fx = num[0];
    parameters->param.cx = num[1];
    parameters->param.fy = num[2];
    parameters->param.cy = num[3];
    parameters->param.k1 = num[4];
    parameters->param.k2 = num[5];
    parameters->param.p1 = num[6];
    parameters->param.p2 = num[7];
    parameters->param.k3 = num[8];
    parameters->param.k4 = num[9];
    parameters->param.k5 = num[10]; 
    parameters->param.k6 = num[11];
  }
...
void K4ACalibrationTransformData::getRgbCameraInfo(sensor_msgs::CameraInfo& camera_info)
{
  ...
  std::ifstream infile("../calibration/color_camera_calibration.txt",std::ios::in);
  if (!infile.fail()){
    double num[15];
    int datalen=0;
    while( ! infile.eof() ){
