    ...
    def calibration(self):
        while True:	
            if self.start_calibration:
                ...
                print 'parameter:'
                print (parameter)
                np.savetxt("../../../Azure_Kinect_ROS_Driver/calibration/color_camera_calibration.txt", parameter,fmt='%f',delimiter=' ')
...
if __name__ == '__main__':
    try:
        # 初始化ros节点
        rospy.init_node("color_camera_callibration")
        rospy.loginfo("Starting color_camera_callibration node")

        camera_caliration(15,10,0.072)
        rospy.spin()

    except KeyboardInterrupt:
        print "Shutting down color_camera_callibration node."
