...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
        ...
        if from_rgb_detection:
            ...
            # import gzip
            with open(overwritten_data_path, 'rb') as fp:
            # with gzip.open(overwritten_data_path, 'rb') as fp:
                # loaded_object = pickle.load(fp)
                # self.id_list = loaded_object[0]
                # self.box2d_list = loaded_object[1]
                # self.box3d_list = loaded_object[2]
                # self.input_list = loaded_object[3]
                # self.label_list = loaded_object[4]
                # self.type_list = loaded_object[5]
                # self.heading_list = loaded_object[6]
                # self.size_list = loaded_object[7]
                # frustum_angle is clockwise angle from positive x-axis
                # self.frustum_angle_list = loaded_object[8]
                self.id_list = pickle.load(fp)
                self.box2d_list = pickle.load(fp)
