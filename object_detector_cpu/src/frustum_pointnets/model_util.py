...
def point_cloud_masking(pts, logits, xyz_only=True):
    ...
    if torch.lt(torch.sum(mask),100):
        choice = np.random.choice(n_pts,int(n_pts),replace=False)
        mask[0,choice] = 1
...
def get_box3d_corners(center, heading_residuals, size_residuals):
    ...
    headings = heading_residuals + heading_bin_centers.view(1,-1) # (bs,12)

    mean_sizes = torch.from_numpy(g_mean_size_arr).float().view(1,NUM_SIZE_CLUSTER,3)\
                 + size_residuals # (1,8,3)+(bs,8,3) = (bs,8,3)
...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, logits, mask_label, \
        ...
        hcls_onehot = torch.eye(NUM_HEADING_BIN)[heading_class_label.long()]  # 32,12
        ...
        scls_onehot = torch.eye(NUM_SIZE_CLUSTER)[size_class_label.long()]  # 32,8
        ...
        predicted_size_residuals_normalized_dist = torch.sum( \
            size_residuals_normalized * scls_onehot_repeat, dim=1)#32,3
        mean_size_arr_expand = torch.from_numpy(g_mean_size_arr).float() \
        ...
        corners_3d = get_box3d_corners(center,\
                    heading_residuals,size_residuals)#(bs,NH,NS,8,3)(32, 12, 8, 8, 3)
        ...
        ###ipdb.set_trace()
        heading_bin_centers = torch.from_numpy(\
            np.arange(0, 2 * np.pi, 2 * np.pi / NUM_HEADING_BIN)).float() # (NH,)
        ...
        mean_sizes = torch.from_numpy(g_mean_size_arr)\
                    .float().view(1,NUM_SIZE_CLUSTER,3)#(1,NS,3)
